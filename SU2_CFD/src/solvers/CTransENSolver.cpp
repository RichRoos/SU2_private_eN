 ...
 * \brief Main subroutines for eN Transition model solver.
 * \author A. Aranake, R. Roos.
...
CTransENSolver::CTransENSolver(CGeometry *geometry, CConfig *config, unsigned short iMesh)
  ...
  nodes = new CTransENVariable(Intermittency_Inf, ReThetaT_Inf, 1.0, 1.0, nPoint, nDim, nVar, config);
...
void CTransENSolver::Postprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh) {
  ...
  for (unsigned long iPoint = 0; iPoint < nPoint; iPoint ++) {
    ...
    nodes -> SetIntermittencyEff(iPoint, nodes->GetIntermittencySep(iPoint));
...
void CTransENSolver::Source_Residual(CGeometry *geometry, CSolver **solver_container,
  ...
  for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
    ...
    /*--- Assign Intermittency Separation ---*/

    nodes ->SetIntermittencySep(iPoint, numerics->GetIntermittencySep());
